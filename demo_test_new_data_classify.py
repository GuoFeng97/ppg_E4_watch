...
from ppg.deep_learn import model_evaluate_classify
...
def test_new_data(demo_data_raw):
    ...
    #ynew,预测的种类值
    ynew = model_evaluate_classify(loaded_model, demo_data_waveforms)

    print(ynew[0])

    ynew_new = []
    for i in range(0, ynew.size-4):
        ynew_new.append( float(ynew[i]+ynew[i+1]+ynew[i+2]+ynew[i+3])/5)
    plt.plot(ynew_new)
    plt.show()
